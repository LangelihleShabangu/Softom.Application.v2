...
{
    public class CreatePaymentReportPDF
    {
        ...
        private PdfPTable HeaderInfo(Document doc, PdfWriter writer)
        {
            ...
            tableLayout.AddCell(new PdfPCell(new Phrase("Member Not Paid", new Font(Font.FontFamily.HELVETICA, 12, 1, iTextSharp.text.BaseColor.BLACK))) { Colspan = 1, Border = 0, PaddingBottom = 10, HorizontalAlignment = Element.ALIGN_CENTER });
        ...
        private PdfPTable AddContent(InvoiceVM invoice, Document doc, PdfWriter writer)
        {
            ...
            string Color = "#FFFFFF";
            //Add header
            AddCellToBody(tableLayout, "Member Name" + " - " + invoice.MemberList.Count().ToString());
            AddCellToBody(tableLayout, "Member Surname");
            AddCellToBody(tableLayout, "Member Phone Number");
            AddCellToBody(tableLayout, "Member Cell Number");

            foreach (var item in invoice.MemberList)
            {
                AddCellToBody(tableLayout, item.ContactInformation.Firstname);
                AddCellToBody(tableLayout, item.ContactInformation.Surname);
                AddCellToBody(tableLayout, item.ContactInformation.PhoneNumber);
                AddCellToBody(tableLayout, item.ContactInformation.CellNumber);
            }
            return tableLayout;
        }
